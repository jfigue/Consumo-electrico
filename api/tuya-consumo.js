...
export default async function handler(req, res) {
  ...
  const deviceId = req.query.device || process.env.TUYA_DEVICE_ID;

  if (!clientId || !secret || !deviceId) {
    return res.status(500).json({ error: "Faltan TUYA_CLIENT_ID / TUYA_CLIENT_SECRET, o no se especificó el dispositivo (?device=)" });
  }
