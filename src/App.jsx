...
const DISPOSITIVOS_TUYA = [
  { id: "eb50ed465ddfd4c630dr1j", nombre: "Enchufe Entrada (Tuya)" },
  { id: "eb2889d17fa12be750ilub", nombre: "Enchufe Ushi (Tuya)" },
];
...
const supaRefresh = (refreshToken) => supaAuth("token?grant_type=refresh_token", { refresh_token: refreshToken });
...
export default function App() {
  ...
  // Enchufes inteligentes (Tuya) — lista de dispositivos
  const [tuyaEstado, setTuyaEstado] = useState({}); // { [deviceId]: { data, cargando, error } }
  const [historialEnchufe, setHistorialEnchufe] = useState([]);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) updateSession(JSON.parse(raw));
    ...
    (async () => {
      ...
      try {
        const [fRows, aRows, cRows] = await Promise.all([
          authedFetch("facturas?select=*"),
          authedFetch("aparatos?select=*"),
          authedFetch("config?select=*"),
        ]);
        ...
        if (cRows && cRows.length > 0) {
          ...
          await authedFetch("config", { method: "POST", body: { tarifa_kwh: CONFIG_DEMO.tarifaKwh } });
  ...
  const authedFetch = useCallback(async (path, options = {}) => {
    try {
      return await supaFetch(path, sessionRef.current, options);
    } catch (err) {
      const msg = String(err.message || "");
      if (!/jwt expired|invalid jwt|401/i.test(msg)) throw err;
      // token vencido: renovamos con el refresh_token y reintentamos una vez
      const refreshed = await supaRefresh(sessionRef.current.refresh_token);
      updateSession(refreshed);
      return await supaFetch(path, refreshed, options);
    }
  }, [updateSession]);
  ...
  const agregarFactura = useCallback(
    async (factura) => {
      const [row] = await authedFetch("facturas", { method: "POST", body: facturaToDb(factura) });
  ...
  const eliminarFactura = useCallback(
    async (id) => {
      await authedFetch(`facturas?id=eq.${id}`, { method: "DELETE", prefer: "return=minimal" });
  ...
  const upsertAparato = useCallback(
    async (aparato) => {
      if (aparato.id) {
        const [row] = await authedFetch(`aparatos?id=eq.${aparato.id}`, {
          method: "PATCH",
          body: aparatoToDb(aparato),
        });
        setAparatos((prev) => prev.map((a) => (a.id === aparato.id ? aparatoFromDb(row) : a)));
      } else {
        const [row] = await authedFetch("aparatos", { method: "POST", body: aparatoToDb(aparato) });
  ...
  const eliminarAparato = useCallback(
    async (id) => {
      await authedFetch(`aparatos?id=eq.${id}`, { method: "DELETE", prefer: "return=minimal" });
  ...
  /* ---------- enchufes inteligentes (Tuya) ---------- */
  const ultimosGuardadosRef = useRef({}); // { [deviceId]: timestamp }

  const leerTuya = useCallback(
    async (deviceId) => {
      setTuyaEstado((prev) => ({ ...prev, [deviceId]: { ...(prev[deviceId] || {}), cargando: true, error: null } }));
      try {
        const res = await fetch(`/api/tuya-consumo?device=${deviceId}`);
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || `Error ${res.status}`);
        setTuyaEstado((prev) => ({ ...prev, [deviceId]: { data, cargando: false, error: null } }));

        // guardamos el historial en Supabase, como máximo cada 5 minutos por dispositivo
        const ahora = Date.now();
        if (ahora - (ultimosGuardadosRef.current[deviceId] || 0) > 5 * 60 * 1000) {
          ultimosGuardadosRef.current[deviceId] = ahora;
          try {
            const [row] = await authedFetch("lecturas_enchufe", {
              method: "POST",
              body: {
                device_id: deviceId,
                leido_en: data.leidoEn,
                potencia_w: data.potenciaW,
                voltaje_v: data.voltajeV,
                corriente_a: data.corrienteA,
                energia_acumulada_kwh: data.energiaAcumuladaKwh,
              },
            });
            if (row) setHistorialEnchufe((prev) => [...prev, row]);
          } catch (err) {
            console.error("No se pudo guardar el historial del enchufe", deviceId, err);
          }
        }
      } catch (err) {
        setTuyaEstado((prev) => ({
          ...prev,
          [deviceId]: { data: prev[deviceId]?.data || null, cargando: false, error: err.message || "No se pudo leer el enchufe" },
        }));
      }
    },
    [authedFetch]
    ...
    (async () => {
      try {
        ...
        const porDispositivo = await Promise.all(
          DISPOSITIVOS_TUYA.map((d) =>
            authedFetch(
              `lecturas_enchufe?select=*&device_id=eq.${d.id}&order=leido_en.desc&limit=500&leido_en=gte.${desde}`
            ).then((rows) => (rows || []).reverse())
          )
        );
        setHistorialEnchufe(porDispositivo.flat());
      } catch (err) {
        console.error("No se pudo cargar el historial del enchufe", err);
      }
    })();
    DISPOSITIVOS_TUYA.forEach((d) => leerTuya(d.id));
    const interval = setInterval(() => DISPOSITIVOS_TUYA.forEach((d) => leerTuya(d.id)), 30000); // lectura en vivo cada 30s
    return () => clearInterval(interval);
  }, [session, leerTuya, authedFetch]);

  // consumo real medido este mes calendario, por dispositivo (para incluir en Análisis)
  const consumoTuyaMesPorDispositivo = useMemo(() => {
    const hoy = new Date();
    const resultado = {};
    for (const d of DISPOSITIVOS_TUYA) {
      const delMes = historialEnchufe.filter((r) => {
        const fecha = new Date(r.leido_en);
        return (
          r.device_id === d.id &&
          fecha.getFullYear() === hoy.getFullYear() &&
          fecha.getMonth() === hoy.getMonth() &&
          r.energia_acumulada_kwh != null
        );
      });
      if (delMes.length < 2) {
        resultado[d.id] = null;
      ...
      const valores = delMes.map((r) => Number(r.energia_acumulada_kwh));
      const delta = Math.max(...valores) - Math.min(...valores);
      resultado[d.id] = delta >= 0 ? delta : null;
    }
    return resultado;
  }, [historialEnchufe]);
  ...
  const aparatosCalc = useMemo(() => {
    const base = aparatos.map((a) => {
      const kwh = consumoAparato(a);
      return { ...a, kwh, costo: kwh * tarifa };
    });
    for (const d of DISPOSITIVOS_TUYA) {
      const kwh = consumoTuyaMesPorDispositivo[d.id];
      if (kwh != null) {
        base.push({
          id: `tuya-medido-${d.id}`,
          nombre: `${d.nombre} (medido)`,
          categoria: "Otros",
          kwh,
          costo: kwh * tarifa,
          medido: true,
    ...
    return base.sort((x, y) => y.kwh - x.kwh);
  }, [aparatos, tarifa, consumoTuyaMesPorDispositivo]);
  ...
  const analizarImagen = async (e) => {
    ...
    try {
      ...
      const response = await fetch("/api/leer-factura", {
        ...
        body: JSON.stringify({ base64, mediaType: media }),
      });
      ...
      console.error(err);
      setAiError("No se pudo leer la factura: " + err.message);
    } finally {
      setAiCargando(false);
      e.target.value = "";
    }
  ...
  const analizarEquipoFoto = async (e) => {
    ...
    try {
      const base64 = await fileToBase64(file);
      const media = file.type && file.type.startsWith("image/") ? file.type : "image/jpeg";
      const response = await fetch("/api/identificar-equipo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ base64, mediaType: media }),
      });
  ...
  const renderDashboard = () => {
    ...
    return (
      <>
        {DISPOSITIVOS_TUYA.map((dispositivo) => {
          const estado = tuyaEstado[dispositivo.id] || {};
          const historialDispositivo = historialEnchufe.filter((r) => r.device_id === dispositivo.id);
          return (
            <Card key={dispositivo.id} style={{ marginBottom: 18, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ width: 38, height: 38, borderRadius: 10, background: "rgba(34,197,94,0.12)", display: "inline-flex", alignItems: "center", justifyContent: "center" }}>
                  <Plug size={19} color={C.success} />
                </span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700 }}>{dispositivo.nombre}</div>
                  {estado.error ? (
                    <div style={{ fontSize: 12, color: C.error }}>{estado.error}</div>
                  ) : estado.data ? (
                    <div style={{ fontSize: 12, color: C.sub }}>
                      Actualizado {new Date(estado.data.leidoEn).toLocaleTimeString("es-AR")}
                    </div>
                  ) : (
                    <div style={{ fontSize: 12, color: C.sub }}>{estado.cargando ? "Leyendo…" : "Sin datos aún"}</div>
                  )}
                </div>
              </div>
              ...
              {estado.data && !estado.error && (
                <div style={{ display: "flex", gap: 22, flexWrap: "wrap" }}>
                  <div>
                    <div style={{ fontSize: 11, color: C.sub }}>Potencia</div>
                    <div style={{ fontSize: 16, fontWeight: 700 }}>{estado.data.potenciaW != null ? `${estado.data.potenciaW.toFixed(1)} W` : "—"}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: C.sub }}>Voltaje</div>
                    <div style={{ fontSize: 16, fontWeight: 700 }}>{estado.data.voltajeV != null ? `${estado.data.voltajeV.toFixed(1)} V` : "—"}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: C.sub }}>Corriente</div>
                    <div style={{ fontSize: 16, fontWeight: 700 }}>{estado.data.corrienteA != null ? `${estado.data.corrienteA.toFixed(2)} A` : "—"}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: C.sub }}>Acumulado</div>
                    <div style={{ fontSize: 16, fontWeight: 700 }}>{estado.data.energiaAcumuladaKwh != null ? `${estado.data.energiaAcumuladaKwh.toFixed(2)} kWh` : "—"}</div>
                  </div>
                </div>
              )}

              <button
                onClick={() => leerTuya(dispositivo.id)}
                disabled={estado.cargando}
                style={{ background: "none", border: `1px solid ${C.border}`, borderRadius: 8, padding: "6px 10px", cursor: "pointer", color: C.sub, fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}
              >
                {estado.cargando ? <Loader2 size={13} className="spin" /> : null}
                Actualizar
              </button>

              {historialDispositivo.length > 1 && (
                <div style={{ width: "100%", height: 130, marginTop: 4 }}>
                  <ResponsiveContainer>
                    <LineChart data={historialDispositivo.map((r) => ({ t: r.leido_en, w: Number(r.potencia_w) }))}>
                      <XAxis
                        dataKey="t"
                        {...axisProps}
                        tickFormatter={(t) => new Date(t).toLocaleDateString("es-AR", { day: "2-digit", month: "2-digit" })}
                        minTickGap={36}
                      />
                      ...
                      <Tooltip
                        {...tipProps}
                        labelFormatter={(t) =>
                          new Date(t).toLocaleString("es-AR", { weekday: "short", day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })
                        }
                        formatter={(v) => [`${v.toFixed(1)} W`, "Potencia"]}
                      />
                      <Line type="monotone" dataKey="w" stroke={C.success} strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              )}
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 720 }}>
            ...
            <tbody>
              {aparatosCalc.filter((a) => !a.medido).map((a, i) => {
