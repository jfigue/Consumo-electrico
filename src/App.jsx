...
export default function App() {
  ...
  const leerTuya = useCallback(async () => {
    setTuyaCargando(true);
    setTuyaError(null);
    try {
      const res = await fetch("/api/tuya-consumo");
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `Error ${res.status}`);
      setTuyaData(data);

      // guardamos el historial en Supabase, como máximo cada 5 minutos
      const ahora = Date.now();
      if (ahora - ultimoGuardadoRef.current > 5 * 60 * 1000) {
        ...
        try {
          ...
          console.error("No se pudo guardar el historial del enchufe", err);
        }
      }
    } catch (err) {
      setTuyaError(err.message || "No se pudo leer el enchufe");
    } finally {
  ...
  useEffect(() => {
    ...
    leerTuya();
    const interval = setInterval(leerTuya, 30000); // lectura en vivo cada 30s
    return () => clearInterval(interval);
  }, [session, leerTuya, authedFetch]);

  // consumo real medido este mes calendario (para incluir en Análisis)
  const consumoTuyaMes = useMemo(() => {
    const hoy = new Date();
    const delMes = historialEnchufe.filter((r) => {
      const d = new Date(r.leido_en);
      return d.getFullYear() === hoy.getFullYear() && d.getMonth() === hoy.getMonth() && r.energia_acumulada_kwh != null;
    });
    if (delMes.length < 2) return null;
  ...
  const aparatosCalc = useMemo(() => {
    ...
    if (consumoTuyaMes != null) {
      base.push({
        id: "tuya-medido",
        nombre: "Enchufe inteligente (medido)",
        categoria: "Otros",
        kwh: consumoTuyaMes,
        costo: consumoTuyaMes * tarifa,
        medido: true,
      });
    }
    return base.sort((x, y) => y.kwh - x.kwh);
  }, [aparatos, tarifa, consumoTuyaMes]);
  ...
  const renderDashboard = () => {
    ...
    return (
      <>
        <Card style={{ marginBottom: 18, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 700 }}>Enchufe inteligente (Tuya)</div>
              {tuyaError ? (
                <div style={{ fontSize: 12, color: C.error }}>{tuyaError}</div>
              ) : tuyaData ? (
                <div style={{ fontSize: 12, color: C.sub }}>
                  Actualizado {new Date(tuyaData.leidoEn).toLocaleTimeString("es-AR")}
                </div>
              ) : (
                <div style={{ fontSize: 12, color: C.sub }}>{tuyaCargando ? "Leyendo…" : "Sin datos aún"}</div>
              )}
            </div>
          </div>
          ...
          {tuyaData && !tuyaError && (
            <div style={{ display: "flex", gap: 22, flexWrap: "wrap" }}>
              <div>
                ...
                <div style={{ fontSize: 16, fontWeight: 700 }}>{tuyaData.potenciaW != null ? `${tuyaData.potenciaW.toFixed(1)} W` : "—"}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: C.sub }}>Voltaje</div>
                <div style={{ fontSize: 16, fontWeight: 700 }}>{tuyaData.voltajeV != null ? `${tuyaData.voltajeV.toFixed(1)} V` : "—"}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: C.sub }}>Corriente</div>
                <div style={{ fontSize: 16, fontWeight: 700 }}>{tuyaData.corrienteA != null ? `${tuyaData.corrienteA.toFixed(2)} A` : "—"}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: C.sub }}>Acumulado</div>
                <div style={{ fontSize: 16, fontWeight: 700 }}>{tuyaData.energiaAcumuladaKwh != null ? `${tuyaData.energiaAcumuladaKwh.toFixed(2)} kWh` : "—"}</div>
              </div>
            </div>
          )}
          ...
          >
            {tuyaCargando ? <Loader2 size={13} className="spin" /> : null}
            Actualizar
          </button>

          {historialEnchufe.length > 1 && (
            <div style={{ width: "100%", height: 90, marginTop: 4 }}>
              <ResponsiveContainer>
                <LineChart data={historialEnchufe.map((r) => ({ t: r.leido_en, w: Number(r.potencia_w) }))}>
                  <XAxis dataKey="t" hide />
                  <YAxis hide domain={["auto", "auto"]} />
                  <Tooltip
                    {...tipProps}
                    labelFormatter={(t) => new Date(t).toLocaleString("es-AR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}
